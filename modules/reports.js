...
async function add(distributionID, distributionDate) {
	...
	*/
	...
	$('#table_first_child').after("<tr><td>" + distributionID + "</td><td>" + distributionDate + "</td><td>" + rows.length + "</td><td>" + normalTotalBytes + "</td><td>" +
	normalTotalAssets + "</td><td><a href='" + distributionID + "--" + distributionDate + ".html'>details</a></td>");
	...
	*/
	...
	const encodedDistributionDate = encodeURIComponent(distributionDate);
	const newItem = `<item>
	<title>Distribution ${distributionID}</title>
	<link>https://wcg.report/${distributionID}--${encodedDistributionDate}.html</link>
	...
	*/
	...
	$('#table_first_child').after("<tr><td>" + row.member_id + "</td><td>" + row.account_name + "</td><td>" + row.score + "</td><td>" +
		Math.round(row.bytes_reward) + "</td><td>" + Math.round(row.diff_from_previous) + "</td><td><a href='https://"+ (process.env.testnet ? 'testnet' : '') +"explorer.obyte.org/#" + row.payout_address + "'>" + row.payout_address + "</a></td><td><a href='https://"+ (process.env.testnet ? 'testnet' : '') +"explorer.obyte.org/#" + row.payment_unit + "'>unit</a></td></tr>");
	});
